...
$phaseConfig = [
    'night' => [
        ...
        'subtitle' => 'Werewolves are hunting...',
...
<div class="glass-panel border {{ $cfg['border'] }} overflow-hidden {{ $narratorView ? '' : 'w-full max-w-2xl mx-auto' }}">
    <div class="bg-gradient-to-r {{ $cfg['gradient'] }} p-4 md:p-5">
        <div class="flex items-start justify-between gap-4">
            <div class="flex items-center gap-3 min-w-0">
                <div class="text-3xl md:text-4xl flex-shrink-0">{{ $cfg['icon'] }}</div>
                <div class="min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        @if($round > 0)
                            <span class="text-xs font-mono {{ $cfg['accent'] }} {{ $cfg['bgAccent'] }} px-2 py-0.5 rounded-full">
                                R{{ $round }}
                            </span>
                        @endif
                    </div>
            ...
            <div class="flex items-center gap-4 flex-shrink-0">
                @if($aliveCount > 0)
                    <div class="text-right">
                        <p class="text-xs text-text-muted uppercase tracking-wider">{{ __('ui.game.players_alive') }}</p>
                        <p class="text-lg font-bold font-mono {{ $phase === 'night' ? 'text-accent-blue' : ($phase === 'voting' ? 'text-accent-red' : 'text-accent-gold') }}">
            ...
            <div class="mt-3 flex items-center gap-2">
                <div class="flex-1 h-1.5 bg-bg-surface rounded-full overflow-hidden">
                    <div class="h-full rounded-full transition-all duration-500 ease-out
                        {{ $phase === 'night' ? 'bg-accent-blue' : ($phase === 'voting' ? 'bg-accent-red' : 'bg-accent-gold') }}"
                        style="width: {{ $pct }}%">
                    </div>
                </div>
                <span class="text-xs text-text-muted font-mono">{{ $pct }}%</span>
            </div>
        @endif
    </div>
</div>
