<div class="glass-panel border border-border-default p-5 w-full animate-fadeInUp"
    ...
    {{-- MASKED STATE: Identical for every player --}}
    @unless($panelOpen)
                        ...
                        <div class="flex gap-3 justify-center pt-2">
                            @if(!$witchSaveUsed)
                                <button wire:click="witchUseSave"
                                        class="flex-1 px-5 py-2.5 bg-accent-gold text-bg-primary font-bold rounded-lg hover:bg-accent-gold-dark transition-all duration-200 text-sm shadow-lg hover:scale-105 active:scale-95">
                                    {{ __('ui.action.save') }}
                                </button>
                            @endif
                            @if(!$witchPoisonUsed)
                                <button wire:click="witchUsePoison"
                                        class="flex-1 px-5 py-2.5 bg-accent-red text-white font-bold rounded-lg hover:bg-accent-red-dark transition-all duration-200 text-sm shadow-lg hover:scale-105 active:scale-95">
                                    {{ __('ui.action.poison') }}
                                </button>
                            @endif
                        </div>
                    ...
                    <div class="text-center py-4 space-y-5">
                        ...
                        <div class="flex gap-4 justify-center">
                            <button wire:click="cancelSelection"
                                    class="px-6 py-2.5 bg-bg-elevated text-text-secondary rounded-lg hover:bg-border-default transition-all duration-200 text-sm font-medium">
                                {{ __('ui.button.cancel') }}
                            </button>
